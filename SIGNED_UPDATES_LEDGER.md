...
| CI-TOOLCHAIN | Primary agent | Correctness | `VaultWorkspaceView.swift`, screenshot tests | Xcode 26.2 rejected the large body expression and newer image-attachment API in CI runs 35018096807 and 35019004357. Separate view expressions and PNG data attachments support the pinned toolchain. CI run 35019595130 also exposed a synchronous layout assumption on macOS 15; the test now waits for each expected layout change. | Verified | Full Swift test and universal build jobs on the pinned compiler | `04e9320`, `85a3682`, `159b266`, and `fa30319` | [Vault #24](https://github.com/lpm-dev/lpm-vault/pull/24) |
...
- Release tooling: 40 shell checks and nine Python tests.
